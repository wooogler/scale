...
concepts:
  - id: pure-math-over-one-input
    name: The estimate is pure arithmetic over a single measured input — the repository's source line count — with the file scan kept outside it
  ...
  - id: build-tier-only-table
    name: Only the two build-tier models appear in the table; the cheap intervention-tier models are deliberately excluded and mentioned only in a footnote
  ...
  - id: rough-by-design
    name: The result is an order-of-magnitude decision aid carrying a stated error band of roughly plus or minus half, with the predicted component count clamped between a floor of five and a ceiling of eighty
rationale:
  - decision: The arithmetic lives in the shared engine as a pure function of one number, while the filesystem scan that produces that number lives in the command layer
...
```mermaid
flowchart LR
    A["repository on disk"] --> B["scan the tree and count source lines"]
    B -->|line count| C["pure estimator"]
    D["one measured real build"] -->|fitted per-line constants| C
    C --> E["estimated component count, clamped"]
