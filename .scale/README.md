...
flowchart TD
    ...
    MEM["coverage memory:<br/>one paper per component,<br/>anchored to source at a fixed revision"]
...
[Coverage Memory](memory/) — read this to learn what the system knows about the codebase: the paper format, the protocol that writes the papers, and the loader that reads the tree back as data.
...
**Charting.** A senior runs a build protocol with a high-capability model against a repository pinned at a fixed revision. It surveys the code, proposes a partition into a few provinces and a few dozen components, stops for approval, then writes one paper per component in parallel — what the component is, named ideas a person could be examined on, and its design reasoning with the provenance of that reasoning recorded. It cross-links the papers and hands geometry to a deterministic command that freezes each position and weight. This runs once and costs real money; everything after is cheap.
