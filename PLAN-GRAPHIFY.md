...
Koa는 36 컴포넌트 / 7 소스파일(**파일당 5.1**)이라 성립하지 않는다.
...
1. **`graphify-check`가 입도 전제를 검사하게 하라.** 컴포넌트/소스파일 비율이 1을 크게
   넘으면 링크 recall·precision을 숫자 대신 "측정 불가 (파일당 N 컴포넌트)"로 출력.
   §10.3의 파일쌍 단위 recall·precision을 대신 계산해 내보내는 것이 정확한 대안이다.
